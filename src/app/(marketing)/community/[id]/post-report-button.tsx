...
export function PostReportButton({ postId }: { postId: string }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      await reportPostAction(postId, fd);
      setSubmitted(true);
      setOpen(false);
    });
  }
  ...
  if (!open) {
    return (
      <button
        ...
        className="cursor-pointer text-sm text-[color:var(--muted)] hover:text-[color:var(--accent)] transition-colors underline underline-offset-4"
      >
        신고
      </button>
    );
